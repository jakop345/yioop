...
class QueueServer implements CrawlConstants
{
    ...
    function loop()
    {
        ...
        while ($info[self::STATUS] != self::STOP_STATE) {
            ...
            if($count < NUM_URLS_QUEUE_RAM - SEEN_URLS_BEFORE_UPDATE_SCHEDULER * MAX_LINKS_PER_PAGE) {
            ...
            if($count > 0) {
                ...
                if($top[1] < MIN_QUEUE_WEIGHT) { 
                    ...
                    $this->web_queue->normalize(); // this will undercount the weights of URLS from fetcher data that have not completed
                 }
    ...
    function handleAdminMessages($info) 
    {
        if(file_exists(CRAWL_DIR."/schedules/queue_server_messages.txt")) {
            $info = unserialize(file_get_contents(CRAWL_DIR."/schedules/queue_server_messages.txt"));
            ...
            switch($info[self::STATUS])
            {
                case "NEW_CRAWL":
                    ...
                    crawlLog("Starting new crawl. Timestamp:".$this->crawl_time);
                break;
                ...
                case "STOP_CRAWL":
                    ...
                    if(isset($this->index_archive)) {
                        ...
                        $this->db->setWorldPermissionsRecursive(CRAWL_DIR.'/cache/'.self::index_data_base_name.$this->crawl_time);
                    }
                ...
                case "RESUME_CRAWL":
                    if(isset($info[self::CRAWL_TIME]) && file_exists(CRAWL_DIR.'/cache/'.self::queue_base_name.$info[self::CRAWL_TIME])) {
                        $this->startCrawl($info);
                        ...
                        if(!file_exists(CRAWL_DIR.'/cache/'.self::queue_base_name.$info[self::CRAWL_TIME])) {
                            $msg .= "queue bundle for crawl to restart does not exist\n";
                        }
                        $info["MESSAGE"] = $msg;
                        $crawl_status = array();
                        ...
                        crawlLog($msg);
                        file_put_contents(CRAWL_DIR."/schedules/crawl_status.txt", serialize($crawl_status));
    ...
    function startCrawl($info)
    {
        ...
        $this->web_queue = new WebQueueBundle(CRAWL_DIR.'/cache/'.self::queue_base_name.$this->crawl_time, URL_FILTER_SIZE, NUM_URLS_QUEUE_RAM, $min_or_max);

        if(!file_exists(CRAWL_DIR.'/cache/'.self::index_data_base_name.$this->crawl_time)) {
            $this->index_archive = new IndexArchiveBundle(CRAWL_DIR.'/cache/'.self::index_data_base_name.$this->crawl_time,
                URL_FILTER_SIZE, NUM_ARCHIVE_PARTITIONS, NUM_INDEX_PARTITIONS, $info['DESCRIPTION']);
        } else {
            $this->index_archive = new IndexArchiveBundle(CRAWL_DIR.'/cache/'.self::index_data_base_name.$this->crawl_time,
                URL_FILTER_SIZE);
        }
        ...
        $this->db->setWorldPermissionsRecursive(CRAWL_DIR.'/cache/'.self::index_data_base_name.$this->crawl_time);
        // initialize and store the description of this crawl in the index archive
    ...
    function deleteOrphanedBundles()
    {
        ...
        foreach($dirs as $dir) {
            if(strlen($pre_timestamp = strstr($dir, self::queue_base_name)) > 0) {
                $timestamp = substr($pre_timestamp, strlen(self::queue_base_name));
                if(!file_exists(CRAWL_DIR.'/cache/'.self::index_data_base_name.$timestamp)) {
    ...
    function processDataFile($base_dir, $callback_method)
    {
        ...
        foreach($dirs as $dir) {
            ...
            if(isset($old_dir)) {
                ...
                /*  The idea is that only go through outer loop more than once if earlier data directory empty
                 *  Note: older directories should only have data dirs or deleting like this might cause problems! 
                 */
            }
    ...
    function processIndexData()
    {
        ...
        $index_dir =  CRAWL_DIR."/schedules/".self::index_data_base_name.$this->crawl_time;
    ...
    {
        static $first = true;
        crawlLog("Start processing index data memory usage".memory_get_usage() . "...");
       ...
       crawlLog("A memory usage".memory_get_usage() .
        ...
        if(isset($seen_sites)) {
            $seen_sites = $index_archive->addPages(self::HASH_URL, self::SUMMARY_OFFSET, $seen_sites);
                ...
                foreach( $index_data as $word_key => $docs_info) {
                    foreach($docs_info as $doc_key => $info) {
                        if(isset($summary_offsets[$doc_key])) {
                            $index_data[$word_key][$doc_key][self::SUMMARY_OFFSET] = $summary_offsets[$doc_key];
                        }
                    }
                }
            }
    ...
    function processRobotUrls()
    {
        ...
        $robot_dir =  CRAWL_DIR."/schedules/".self::robot_data_base_name.$this->crawl_time;
        ...
        if(isset($sites)) {
            foreach($sites as $robot_host => $robot_info) {
                ...
                if(isset($robot_info[self::CRAWL_DELAY])) {
                    $this->web_queue->setCrawlDelay($robot_host, $robot_info[self::CRAWL_DELAY]);
                }

                if(isset($robot_info[self::ROBOT_PATHS])) {
                    foreach($robot_info[self::ROBOT_PATHS] as $path) {
                        $this->web_queue->addDisallowedRobotFilter($robot_host.$path); 
                    }
                }
            }
        }
    ...
    function deleteRobotData()
    {
        ...
        $this->db->unlinkRecursive(CRAWL_DIR.'/schedules/'.self::robot_data_base_name.$this->crawl_time, true);
    ...
    {
        crawlLog("Start checking for new URLs data memory usage".memory_get_usage());
        ...
        $schedule_dir =  CRAWL_DIR."/schedules/".self::schedule_data_base_name.$this->crawl_time;
    ...
    function processDataArchive($file)
    {
        ...
        if(isset($sites[self::SCHEDULE_TIME])) {
            if(isset($this->waiting_hosts[$sites[self::SCHEDULE_TIME]])) {
                $delayed_hosts = $this->waiting_hosts[$sites[self::SCHEDULE_TIME]];
                unset($this->waiting_hosts[$sites[self::SCHEDULE_TIME]]);
                foreach($delayed_hosts as $hash_host) {
                    unset($this->waiting_hosts[$hash_host]); //allows crawl-delayed host to be scheduled again
                }
            }
        }
        crawlLog(" time: ".(changeInMicrotime($start_time)));
        ...
        if(isset($sites[self::SEEN_URLS])) {
            ...
            foreach($sites[self::SEEN_URLS] as $url) {
                if($this->web_queue->containsUrlQueue($url)) {
                    crawlLog("Removing $url from Queue (shouldn't still be there!)");
        ...
        if(isset($sites[self::TO_CRAWL])) {
            ...
            foreach($to_crawl_sites as $pair) {
                ...
                $robots_in_queue = $this->web_queue->containsUrlQueue($host_with_robots);

                if($this->web_queue->containsUrlQueue($url)) {

                    if($robots_in_queue) {
                        $this->web_queue->adjustQueueWeight($host_with_robots, $weight);
                    }
                    $this->web_queue->adjustQueueWeight($url, $weight);
                } else if($this->allowedToCrawlSite($url) && !$this->disallowedToCrawlSite($url)  ) {
        ...
        crawlLog("The current crawl description is: ".$info_bundle['DESCRIPTION']);
    ...
    function produceFetchBatch()
    {
        ...
        $sites[self::SAVED_CRAWL_TIMES] =  $this->getCrawlTimes(); // fetcher should delete any crawl time not listed here
        $sites[self::CRAWL_ORDER] = $this->crawl_order;
        $sites[self::SITES] = array();

        $delete_urls = array();
        $crawl_delay_hosts = array();
        $time_per_request_guess = MINIMUM_FETCH_LOOP_TIME ; // it would be impressive if we can achieve this speed
        ...
        while ($i <= $count && $fetch_size < MAX_FETCH_SIZE) {
            ...
            if(strcmp($host_url."/robots.txt", $url) == 0) {
                if($this->web_queue->containsGotRobotTxt($host_url)) {
                    ...
                    $next_slot = $this->getEarliestSlot($current_crawl_index, $sites[self::SITES]);
                    
                    if($next_slot < MAX_FETCH_SIZE) {
                        $sites[self::SITES][$next_slot] = array($url, $weight);
                        $delete_urls[$i] = $url;
                        // note don't add to seen url filter since check robots every 24 hours as needed
            ...
            if($this->web_queue->containsGotRobotTxt($host_url)) {
                ...
                foreach($host_paths as $host_path) {
                    if($this->web_queue->containsDisallowedRobot($host_path)) {
                        $robots_okay = false;
                        $delete_urls[$i] = $url; //we want to remove from queue since robots forbid it
                        $this->web_queue->addSeenUrlFilter($url); 
                            // at this point we might miss some sites by marking them seen: the robot url might change in 24 hours
                        break;
                    }
                }
                ...
                if($delay > 0 ) { // handle adding a url if there is a crawl delay
                    if((!isset($this->waiting_hosts[crawlHash($host_url)])
                        && $num_waiting < MAX_WAITING_HOSTS)
                        || (isset($this->waiting_hosts[crawlHash($host_url)]) &&
                            $this->waiting_hosts[crawlHash($host_url) ] == $sites[self::SCHEDULE_TIME])) {

                        $this->waiting_hosts[crawlHash($host_url)] = $sites[self::SCHEDULE_TIME];
                        $this->waiting_hosts[$sites[self::SCHEDULE_TIME]][] = crawlHash($host_url);
                        ...
                        if(!isset($crawl_delay_hosts[$host_url])) {
                            ...
                            $next_earliest_slot = $crawl_delay_hosts[$host_url] + $request_batches_per_delay*NUM_MULTI_CURL_PAGES;
                        }
                        
                        if(($next_slot = $this->getEarliestSlot($next_earliest_slot, $sites[self::SITES])) < MAX_FETCH_SIZE) {
                            $crawl_delay_hosts[$host_url] = $next_slot;
                            $sites[self::SITES][$next_slot] = array($url, $weight);
                            $delete_urls[$i] = $url;
                            $this->web_queue->addSeenUrlFilter($url); // we might miss some sites by marking them seen after only scheduling them
        ...
        if(isset($sites[self::SITES]) && count($sites[self::SITES]) > 0 ) {
            $dummy_slot = array(self::DUMMY, 0.0);  // dummy's are used for crawl delays of sites with longer delays
            ...
            file_put_contents(CRAWL_DIR."/schedules/schedule.txt", serialize($sites));
                crawlLog("End Produce Fetch Memory usage".memory_get_usage() );
                crawlLog("Created fetch batch... Queue size is now ".$this->web_queue->to_crawl_queue->count."...Time to create batch: ".(changeInMicrotime($start_time)));
        } else {
    ...
    function urlMemberSiteArray($url, $site_array)
    {
        ...
        foreach($site_array as $site) {
            ...
            if(isset($site_parts[1]) && mb_strstr(UrlParser::getHost($url), $site_parts[1]) ) {
                $flag = true;
                break;
            }
    ...
    function getCrawlTimes()
    {
        ...
        foreach($dirs as $dir) {
            if(strlen($pre_timestamp = strstr($dir, self::index_data_base_name)) > 0) {
                $list[] = substr($pre_timestamp, strlen(self::index_data_base_name));
            }
        }
