 ...
 */
require_once BASE_DIR . "/lib/process_wordnet.php";
...
炒饭 炒飯 cặc lồn kaltak orospu siktir sıçmak amcık";
    ...
    {
        $replacements = array();
        $num_non_zero_score = array();
        $num_zero_score = array();
        $string = PartOfSpeechTagger::getPartOfSpeechTagging($orig_query);
        $replacements =
        WordNetProcessor::processWordNetOutput(trim($orig_query),trim($string));
        $i = 0;
        $cnt = 0;
        foreach($replacements as $replacement) {
            $cnt =
            self::compute_score($replacement, $threshold, $index_name, $lang);
            if($cnt){
                $num_non_zero_score[$replacement] = $cnt;
            } else {
                $num_zero_score[$replacement] = $cnt;
            }
            $i++;
        }
        $result = array();
        if(count($num_non_zero_score) >= 2) {
            $i = 0;
            arsort($num_non_zero_score);
            foreach($num_non_zero_score as $k => $v) {
                $result[$i] = $k;
                $i++;
                if($i == 2)
                    break;
            }
        } elseif (count($num_non_zero_score) == 1) {
            $i = 0;
            foreach($num_non_zero_score as $k => $v) {
                $result[$i] = $k;
                $i++;
    ...
    /**
     *  Computes count for each word in posting list
     *  @param string $replacement input query part of speech tagging
     *  @param integer $threshold once count in posting list for any word
     *      reaches to threshold then return the number
     *  @param string $index_name selected index for search engine
     *  @param string $lang language
     *  @return integer $count_whole_phrase in posting list for given word
     */
    static function compute_score($replacement, $threshold, $index_name, $lang)
    {
        $string = $replacement;
        if (isset(self::$programming_language_map[$lang])) {
            $control_word =
            self::$programming_language_map[$lang].self::CONTROL_WORD_INDICATOR;
            $string = trim(substr($string, strlen($control_word) + 1));
        } else {
            self::canonicalizePunctuatedTerms($string, $lang);
        }
        $terms = self::stemCharGramSegment($string, $lang);
        $num   = count($terms);
        /*  Changed as term size will not be zero.
         *  if ($index_name == NULL || $num <= 1) {
         */
        if ($index_name == NULL) {
            return $terms;
        ...
        $count_whole_phrase =
        IndexManager::numDocsTerm($whole_phrase, $index_name, $threshold);
        return $count_whole_phrase;
    }
}