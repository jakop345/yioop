...
class FetchGitRepositoryUrls implements CrawlConstants
{
     ...
     * @var array
     */
    static $repository_types = array('git' => 'git', 'svn' => 'svn',
     ...
     * extension of urls
     */
    static function checkForRepository($extension)
    {
        $repository_type;
     ...
     */
    static function setGitRepositoryUrl($url_to_check, $counter, $seeds,
        $repository_indicator, $site_pair, $total_git_urls, $all_git_urls)
    {
        $git_internal_urls = array();
        if(!strpos($url_to_check, self::GIT_URL_CONTINUE)) {
            ...
            if(intval(NUM_MULTI_CURL_PAGES) - $counter < $total_git_urls) {
                $total_git_urls = intval(NUM_MULTI_CURL_PAGES) - $counter;
            }
    ...
    static function fetchGitRepositoryUrl($url_to_check)
    {
        ...
        $git_base_url = substr($url_to_check, self::GIT_BASE_URL_START,
            $position);
        $base_url_last_letter = substr($git_base_url, self::
            GIT_BASE_URL_END_POSITION, self::GIT_BASE_END_LETTER);
     ...
     */
    static function getGitMasterFile($git_first_url_content, $git_base_url)
    {
     ...
     */
    static function getGitMasterTree($git_second_url_content, $git_base_url)
    {
     ...
     * url
     */
    static function getNextGitUrl($git_url, $compression_indicator)
    {
     ...
     */
    static function getObjects($git_object_content, $git_base_url)
    {
        $blob_url = array();
     ...
     */
    static function readBlobSha($git_object_content, $blob_position, $length,
        $git_base_url)
    {
        $git_blob_content = array();
        $blob_values = array();
     ...
     */
    static function readTreeSha($git_object_content, $tree_position, $length,
        $git_base_url)
    {
        $git_tree_content = array();
        $tree_values = array();
     ...
     */
    static function checkNestedStructure($sha_hash, $git_base_url)
    {
     ...
     */
    static function urlMaker($sha_hash, $git_base_url)
    {
