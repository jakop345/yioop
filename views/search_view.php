...
class SearchView extends View implements CrawlConstants
{
    ...
    public function renderView($data) 
    {
        ...
        ?>
        <h1 class="logo"><a href="."><img src="resources/yioop.png" alt="Yioop!" /></a></h1>
        <div class="searchbox">
        ...
        if(!isset($data['PAGES'])) {
            ...
            ?>
            <h2><?php e(tl('search_view_query_results')); ?> (<?php e(tl('search_view_calculated', $data['ELAPSED_TIME']));?> <?php
              e(tl('search_view_results', $data['LIMIT'], min($data['TOTAL_ROWS'], $data['LIMIT'] + $data['RESULTS_PER_PAGE']), $data['TOTAL_ROWS'])); 
            ?> )</h2>
            <?php
            foreach($data['PAGES'] as $page) {?> 
                ...
                <a href="<?php e($page[self::URL]); ?>" ><?php
                 if(isset($page[self::THUMB]) && $page[self::THUMB] != 'NULL') {
                    ?><img src="<?php e($page[self::THUMB]); ?>" alt="<?php e($page[self::TITLE]); ?>"  /> <?php
                 } else {
                ...
                <p class="echolink" ><?php e($page[self::URL]." "); e(tl('search_view_rank', number_format($page[self::DOC_RANK], 2))); 
                e(tl('search_view_relevancy',number_format(1.25*floatval($page[self::SCORE]) - floatval($page[self::DOC_RANK]), 2) )); 
                e(tl('search_view_score', 1.25* $page[self::SCORE]));?>
                <a href="?c=search&amp;a=cache&amp;q=<?php e($data['QUERY']); ?>&amp;arg=<?php e(urlencode($page[self::URL])); 
                    ?>&amp;so=<?php e(urlencode($page[self::SUMMARY_OFFSET])); ?>" >
                <?php
                ...
                ?></a>. <a href="?c=search&amp;a=related&amp;arg=<?php e(urlencode($page[self::URL])); ?>&amp;so=<?php e(urlencode($page[self::SUMMARY_OFFSET])); ?>" ><?php e(tl('search_view_similar')); ?></a>.</p>
                </div>

            <?php 
            } //end foreach
            $this->paginationHelper->render($data['PAGING_QUERY'], $data['LIMIT'], $data['RESULTS_PER_PAGE'], $data['TOTAL_ROWS']);
        }
    }
}
?>
