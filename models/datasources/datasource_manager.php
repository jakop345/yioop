...
abstract class DatasourceManager
{
     ...
     *  
     *  @param string $db_url the url of where the database is located (not used in all dbms's)
     *  @param string $db_user the user to connect as
     *  @param string $db_password the password of the user to connect as
     *  @return mixed return false if not successful and some kind of connection object/identifier otherwise
     */
    ...
    /**
     *  Hook Method for execute(). Executes the supplied sql command on the database
     *
     ...
     *
     * @param string $dir Directory name
     * @param boolean $chmodRootToo Delete specified top-level directory as well
     */
     ...
     *
     * It would have been coon to use traverseDirectory to implement this,
     * but it was a little bit too much of a stretch to showhorn the code to match
     *
        ...
        while(false !== ( $obj = readdir($dh)) ) {
            if (( $obj != '.' ) && ( $obj != '..' )) {
                if ( is_dir($source_dir . '/' . $obj) ) {
                    $this->copyRecursive($source_dir . '/' . $obj, $destination_dir . '/' . $obj);
                }
                else {
                    copy($source_dir . '/' . $obj, $destination_dir . '/' . $obj);
