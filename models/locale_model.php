...
class LocaleModel extends Model 
{
    ...
    var $locale_name;
    /**
     * Combination of text direction and block porgression as a string. Has one of four values:
     * lr-tb, rl-tb, tb-lr, tb-rl. Other possible values for things like Arabic block quoted in Mongolian not supported
     * @var string
     */
    var $writing_mode;
    /**
    ...
    /**
     *  Loads the provided locale's configure file (containing transalation) and calls setlocale to
     *  set up locale specific string formatting (for to format numbers, etc.)
     *
     *  @param string $locale_tag  the tag of the locale to use as the current locale
     */
    function initialize($locale_tag)
    {
        $this->db->selectDB(DB_NAME);

        $this->configure = parse_ini_file (LOCALE_DIR."/$locale_tag/configure.ini", true);
        ...
        setlocale(LC_ALL, $locale_tag, $locale_tag.'.UTF-8', $locale_tag.'.UTF8',  $locale_tag.".TCVN", $locale_tag.".VISCII",
            $locale_tag_parts[0], $locale_tag_parts[0].'.UTF-8', $locale_tag_parts[0].'.UTF8', $locale_tag_parts[0].".TCVN");
    ...
    function getLocaleList()
    {
        ...
        while($locales[$i] = $this->db->fetchArray($result)) {
            //the statistics text file contains info used to calculate what fraction of strings have been translated
            if(!file_exists(LOCALE_DIR."/{$locales[$i]['LOCALE_TAG']}/statistics.txt") ||
                filemtime(LOCALE_DIR."/{$locales[$i]['LOCALE_TAG']}/statistics.txt") <
                filemtime(LOCALE_DIR."/{$locales[$i]['LOCALE_TAG']}/configure.ini")) {

                $tmp = parse_ini_file (LOCALE_DIR."/{$locales[$i]['LOCALE_TAG']}/configure.ini", true);
                $num_ids = 0;
                $num_strings = 0;
                ...
                $locales[$i]['PERCENT_WITH_STRINGS'] = floor(100 * $num_strings/$num_ids);
                file_put_contents(LOCALE_DIR."/{$locales[$i]['LOCALE_TAG']}/statistics.txt",
                    serialize($locales[$i]['PERCENT_WITH_STRINGS']));
            } else {
                $locales[$i]['PERCENT_WITH_STRINGS'] =
                    unserialize(file_get_contents(LOCALE_DIR."/{$locales[$i]['LOCALE_TAG']}/statistics.txt"));
            }
            $i++;
     ...
     *
     *  @param string $locale_name  the name of the locale in the locale's language
     *  @param string $locale_tag  the IANA langauge tag for the locale
     *  @param string $writing_mode  a combination of the horizontal and vertical text direction used for writing in the locale
     */
    function addLocale($locale_name, $locale_tag, $writing_mode)
    {
        $this->db->selectDB(DB_NAME);

        $sql = "INSERT INTO LOCALE(LOCALE_NAME, LOCALE_TAG, WRITING_MODE) VALUES".
            "('".$this->db->escapeString($locale_name)."', '".$this->db->escapeString($locale_tag) .
    ...
    function deleteLocale($locale_tag)
    {
        ...
        $sql = "DELETE FROM LOCALE WHERE LOCALE_TAG = '".$this->db->escapeString($locale_tag)."'";
    ...
    /**
     *  For each translatable identifier string (either static from a configure ini file, or dynamic from the db)
     *  return its name together with its translation into the given locale if such a translation exists.
     *
    ...
    function getStringData($locale_tag)
    {
        ...
        if(in_array(DBMS, array('sqlite', 'sqlite3'))) {
            $sql = "SELECT T.IDENTIFIER_STRING AS MSG_ID, TLL.TRANSLATION AS MSG_STRING " . 
                "FROM TRANSLATION T LEFT JOIN ". //sqlite supports left but not right outer join
                "(TRANSLATION_LOCALE TL JOIN LOCALE L ON L.LOCALE_TAG = '$locale_tag' AND L.LOCALE_ID = TL.LOCALE_ID) TLL " .
                "ON T.TRANSLATION_ID = TLL.TRANSLATION_ID";
        } else {
            $sql = "SELECT T.IDENTIFIER_STRING AS MSG_ID, TL.TRANSLATION AS MSG_STRING " . 
                "FROM TRANSLATION T LEFT JOIN ". 
                "(TRANSLATION_LOCALE TL JOIN LOCALE L ON L.LOCALE_TAG = '$locale_tag' AND L.LOCALE_ID = TL.LOCALE_ID) " .
    ...
    function updateStringData($locale_tag, $new_strings)
    {
        ...
        foreach($new_strings as $msg_id => $msg_string) {
            if(strcmp($msg_id, strstr($msg_id, "db_")) == 0) {
                $sql = "SELECT TRANSLATION_ID FROM TRANSLATION WHERE IDENTIFIER_STRING = '$msg_id' LIMIT 1";
                $result = $this->db->execute($sql);
                $row = $this->db->fetchArray($result);

                $translate_id = $row['TRANSLATION_ID'];

                $sql = "DELETE FROM TRANSLATION_LOCALE WHERE TRANSLATION_ID ='$translate_id' AND LOCALE_ID = '$locale_id'";
                $result = $this->db->execute($sql);

                $sql = "INSERT INTO TRANSLATION_LOCALE VALUES ('$translate_id', '$locale_id', '$msg_string')";
                $result = $this->db->execute($sql);
     ...
     *
     *  Suppose the identifier string was some_view_fraction_received and two additional arguments 5 and 10 were given. 
     *  Suppose further that its translation into the current locale (say en_US) was "%s out of %s". Then the string returned by translate
     *  would be "5 out of 10".
     *
     *  @param array $arr an array consisting of an identifier string followed optionally by parameter values.
     *  @return string the translation of the identifier string into the current locale where all %s have been replaced by the corresponding
     *      parameter values
     */
     ...
     *
     * @return string a direction which is one of tb -- top-bottom, rl -- right-to-left, or lr -- left-to-right
     */
    ...
    /**
     *  Used to extract identifier strings from files with correct extensions, then these
     *  strings are merged with existing extracted strings for each locale as well as their
     *  translations (if an extract string has a translation the translation is untouched by this process).
     *
     *  @return array a pair consisting of the data from the general.ini file together with an array of msg_ids msg_strings.
     */
    function extractMergeLocales()
    {
        $strings = $this->getTranslateStrings($this->extract_dirs, $this->extensions) ;
     ...
     *  documents that might need to be translation, the old configure.ini file (this might have existing translations), 
     *  as well as new translation data that might come from a localizer via a web form and 
     ...
     *  @param array $new_configure  translations of identifier strings from another source such as a localizer using a web form
     */
    function updateLocale($general_ini, $strings, $dir, $locale, $new_configure = NULL)
    {
        $old_configure = array();
        $cur_path = $dir . '/' . $locale;
            ...
            if(is_array($general_value)) {
                ...
                foreach($general_value as $name => $value) {
                    if(isset($new_configure[$general_name][$name])) {
                        $n[] = $name.' = "'.addslashes($new_configure[$general_name][$name]).'"';
                ...
                if(isset($new_configure[$general_name])) {
                    $n[] = $general_name.' = "'.addslashes($new_configure[$general_name]).'"';
        ...
        foreach($strings as $string) {
            if( isset($string[0]) && $string[0] == ";") {
                ...
                if(isset($new_configure['strings'][$string])) {
                    $n[] = $string.' = "'.addslashes($new_configure['strings'][$string]).'"';
    ...
    function traverseExtractRecursive($dir, $extensions) 
    {
      ...
      if(!$dh = @opendir($dir)) {
        ...
        while (($obj = readdir($dh)) !== false) {
         ...
         $cur_path = $dir . '/' . $obj;
         if (is_dir($cur_path)) {
             $dir_strings = $this->traverseExtractRecursive($cur_path, $extensions);
             if(count($dir_strings) > 0) {
                  $strings[] = ";";
                  $strings[] = "; $cur_path";
                  $strings = array_merge($strings, $dir_strings);
